...
# Update the GPT-4All model initialization to use the Groovy text generation model
try:
    model_path = os.path.join(os.getcwd(), "resources", "ggml-gpt4all-j-v1.3-groovy.bin")  # Use the Groovy model for text generation
...
def main():
    ...
    def generate_response():
        ...
        try:
            ...
            prompt = ("Expand the following response into a detailed and professional draft letter:\n\n"
                      f"{response_content}\n\n"
            ...
            # Truncate prompt if too long for model context window (e.g., 2000 chars)
            max_prompt_length = 2000
            if len(prompt) > max_prompt_length:
                messagebox.showerror("Error", "The generated prompt is too long for the model. Please shorten your input or use a shorter template.")
                return
