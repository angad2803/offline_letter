...
def main():
    ...
    redact_name_var = tk.BooleanVar(value=False)
    redact_achievements_var = tk.BooleanVar(value=False)

    def redact_fields(text, redact_name=True, redact_achievements=True):
        import re
        if redact_name:
            # Replace lines with 'Name:' or similar
            text = re.sub(r'(Name\s*:\s*)(.*)', r'\1[REDACTED]', text, flags=re.IGNORECASE)
            text = re.sub(r'(Pers No, RK &Name\s*:\s*)(.*)', r'\1[REDACTED]', text, flags=re.IGNORECASE)
        if redact_achievements:
            # Replace lines with 'achievement' or similar
            text = re.sub(r'(?i)achievements?:.*', 'Achievements: [REDACTED]', text)
        return text
