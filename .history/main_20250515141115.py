...
# Update the GPT-4All model initialization to use the Falcon text generation model and CPU backend
try:
    model_path = os.path.join(os.getcwd(), "resources", "gpt4all-falcon-q4_0.gguf")  # Use the Falcon model for text generation
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model file not found at {model_path}. Please ensure it exists.")
    gpt_model = GPT4All(model_path)  # Removed backend argument for compatibility
...
def main():
    ...
    def generate_response():
        ...
        try:
            from gpt4all import GPT4All
            gpt = GPT4All(model_path)  # Removed backend argument for compatibility
            gpt.open()
